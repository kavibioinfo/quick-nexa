...
// ==========================================
// 👑 AYUSHNEXA TRUE 4-COLUMN PRODUCTION PIPELINE CONFIG (100% MATCHED)
// ==========================================
const GOOGLE_FORM_ACTION = "https://docs.google.com/forms/u/0/d/e/1FAIpQLSd5Ouan0XbToloIZLbN0CW837SkudsguZjbYh74A/formResponse"; 

const ENTRY_IDS = {
  businessName: "entry.1978475675",
  ownerName:    "entry.19381357",   // कॉलम C: मालकाचे नाव + मुख्य व्हॉट्सॲप
  address:      "entry.592958278",  // कॉलम D: पत्ता + कॅटेगरी
  services:     "entry.1779922914", // कॉलम E: सेवा + कलर्स + बॅकअप नंबर
};
// ==========================================
      ...
      <div className="border-b border-gray-100" style={{ background: "linear-gradient(135deg, #EFF6FF 0%, #F0F9FF 50%, #F5F3FF 100%)" }}>
        <div className="max-w-5xl mx-auto px-4 py-12 text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-black leading-tight mb-4" style={{ fontFamily: "'DM Serif Display', serif", letterSpacing: "-0.02em" }}>
            Premium Website<br /><span style={{ background: "linear-gradient(135deg, #1E40AF, #2563EB)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Onboarding Form</span>
          </h1>
          ...
          <div className="flex flex-wrap items-center justify-center gap-4 text-xs text-gray-500">
            {["✓ 100% Secure", "✓ WhatsApp Automation", "✓ Domain Included", "✓ 6 Months Support"].map((t) => (<span key={t} className="font-medium">{t}</span>))}
          </div>
        </div>
      </div>
            ...
            {STEPS.map((s) => {
              ...
              return (
                <div key={s.id} className="flex flex-col items-center gap-1.5 relative z-10">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 border-2" style={{ background: done || active ? "linear-gradient(135deg, #1E40AF, #3B82F6)" : "#fff", borderColor: done || active ? "#1E40AF" : "#E5E7EB" }}>
      ...
      <div className="max-w-2xl mx-auto px-4 py-10">
            ...
            <div className="pt-2">
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {["Dental / Clinic", "Gym / Fitness", "Salon / Spa", "Coaching / Education", "Restaurant / Hotel", "Other"].map((type) => (
                  <button key={type} type="button" onClick={() => update("websiteType", type)} className="px-4 py-3 rounded-xl border text-sm font-medium text-left" style={{ background: form.websiteType === type ? "#EFF6FF" : "#FAFAFA", borderColor: form.websiteType === type ? "#1E40AF" : "#E5E7EB", color: form.websiteType === type ? "#1E40AF" : "#374151" }}>{type}</button>
                ))}
              </div>
            </div>
          </div>
        ...
        {step === 2 && (
          <div className="space-y-5">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase mb-2"><Globe size={13} className="inline mr-1" />Your Services / Products *</label>
              <textarea required placeholder="Describe what you offer..." value={form.services} onChange={(e) => update("services", e.target.value)} rows={5} className="w-full rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3.5 text-sm text-gray-800 outline-none resize-none" />
            </div>
        ...
        {step === 3 && (
          <div className="space-y-5">
            ...
            <label htmlFor="file-upload" className={`flex flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed p-10 cursor-pointer ${dragActive ? "border-blue-400 bg-blue-50" : "border-gray-200 bg-gray-50"}`} onDragOver={(e) => { e.preventDefault(); setDragActive(true); }} onDragLeave={() => setDragActive(false)} onDrop={handleDrop}>
            ...
            {files.length > 0 && <div className="text-xs text-gray-600 bg-gray-50 p-2 rounded-lg">{files.join(", ")}</div>}
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase mb-2.5"><Palette size={13} className="inline mr-1" />Preferred Colors</label>
              <div className="grid grid-cols-3 gap-2.5">
                {[{ label: "Navy Blue", colors: ["#1E3A8A", "#3B82F6"] }, { label: "Black & Gold", colors: ["#111827", "#D97706"] }, { label: "Forest Green", colors: ["#14532D", "#22C55E"] }].map((c) => (
                  <button key={c.label} type="button" onClick={() => update("colors", c.label)} className="rounded-xl border-2 overflow-hidden" style={{ borderColor: form.colors === c.label ? "#1E40AF" : "transparent" }}>
                    <div className="h-8 flex"><div className="flex-1" style={{ background: c.colors[0] }} /><div className="flex-1" style={{ background: c.colors[1] }} /></div>
        ...
        <div className="flex items-center justify-between mt-10 pt-6 border-t border-gray-100">
          {step > 1 ? <button type="button" onClick={() => setStep((s) => s - 1)} className="px-5 py-2.5 border rounded-xl text-sm font-semibold text-gray-600">← Back</button> : <div />}
          {step < STEPS.length ? (
            <button type="button" onClick={() => setStep((s) => s + 1)} className="flex items-center gap-2 px-7 py-3 text-white text-sm font-bold rounded-xl shadow-lg" style={{ background: "linear-gradient(135deg, #1E40AF, #3B82F6)" }}>Continue <ChevronRight size={16} /></button>
          ) : (
            <button type="button" disabled={loading} onClick={handleFormSubmit} className="flex items-center gap-2 px-7 py-3 text-white text-sm font-bold rounded-xl shadow-lg disabled:opacity-50" style={{ background: "linear-gradient(135deg, #1E40AF, #3B82F6)" }}><Send size={15} />{loading ? "Submitting..." : "Submit Information"}</button>
          )}
        </div>
      </div>
    </div>
...
function FormField({ icon, label, required, children }) {
  return (
    <div>
      <label className="block text-xs font-semibold text-gray-500 uppercase mb-2"><span className="inline-flex items-center gap-1">{icon} {label} {required && <span className="text-blue-600">*</span>}</span></label>
      <div className="flex items-center gap-3 rounded-xl border bg-gray-50 px-4 py-3.5 focus-within:bg-white transition-all"><span className="text-blue-500">{icon}</span>{children}</div>
    </div>
  );
}